...
class CertificateLightDetailFragment : Fragment(R.layout.fragment_certificate_light_detail) {
	...
	private fun displayErrorState(state: VerificationState.ERROR) {
		...
		if (state.isOfflineMode()) {
			binding.certificateLightDetailVerificationStatus.text =
				requireContext().getString(R.string.wallet_homescreen_offline).makeBold()
		} else {
			binding.certificateLightDetailVerificationStatus.text =
				requireContext().getString(R.string.wallet_homescreen_network_error).makeBold()
		}
	}
